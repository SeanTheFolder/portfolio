...
/* RADAR */
let rdrChart=null;
function rdrColors(){const dark=document.documentElement.getAttribute('data-theme')==='dark';return{acc:dark?'rgba(219,72,98,1)':'rgba(139,26,47,1)',fill:dark?'rgba(219,72,98,.18)':'rgba(139,26,47,.12)',grid:dark?'rgba(61,32,40,1)':'rgba(232,208,212,1)',txt:dark?'#c4a0a8':'#6b4550'};}
...
function loadChartJs(cb){if(typeof Chart!=='undefined'){cb();return;}if(chartJsLoading)return;chartJsLoading=true;const s=document.createElement('script');s.src='https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.0/chart.umd.min.js';s.onload=()=>{chartJsLoading=false;cb();};s.onerror=()=>{chartJsLoading=false;};document.head.appendChild(s);}
function initRadar(){const el=document.getElementById('radarChart');if(!el||rdrChart||window.innerWidth<=479)return;if(typeof Chart==='undefined'){loadChartJs(initRadar);return;}const c=rdrColors();rdrChart=new Chart(el,{type:'radar',data:{labels:rdrLabels,datasets:[{data:rdrScores,backgroundColor:c.fill,borderColor:c.acc,borderWidth:2,pointBackgroundColor:c.acc,pointRadius:4}]},options:{responsive:true,maintainAspectRatio:true,scales:{r:{min:60,max:100,ticks:{display:false},grid:{color:c.grid},angleLines:{color:c.grid},pointLabels:{color:c.txt,font:{size:9.5,weight:'600',family:'DM Sans'}}}},plugins:{legend:{display:false}}}});}
